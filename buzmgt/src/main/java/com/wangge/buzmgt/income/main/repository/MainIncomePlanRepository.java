...
public interface MainIncomePlanRepository extends JpaRepository<MainIncomePlan, Long> {
   ...
   * @since JDK 1.8
   */
  @Query(value = "select  c.BRAND_ID brandId,c.MACHINE_TYPE machineType,c.name from sys_sj_brand_code c where c.MACHINE_TYPE like %?1%", nativeQuery = true)
