...
public class LogServiceImpl implements LogService {
  ...
  public void log(Object origin, Object now, Log.EventType eventType) {
    ...
    log.setEventUsername(
        ObjectUtils.isEmpty(EnvironmentUtils.getUser()) ? "" : EnvironmentUtils.getUser().getUsername());
    ...
    log.setOrigin(ObjectUtils.isEmpty(origin) ? "" : origin.toString()); // 先用
                                                                         // entity.toString方法来作为参数,现在阶段的entity关系太复杂,很有可能出现栈溢出
    
    log.setNow(ObjectUtils.isEmpty(now) ? "" : now.toString()); // 先用
                                                                // entity.toString方法来作为参数,现在阶段的entity关系太复杂,很有可能出现栈溢出
