...
public interface CashRepository extends JpaRepository<Cash, Long>, JpaSpecificationExecutor<Cash> {
  ...
  List<Cash> findByUserIdAndStatusAndFlag(String userId, CashStatusEnum status, FlagEnum flagEnum);

  /**
   * 
   * @Title: findByStatusGroupByUserId @Description: 查询为结算的用户id @param @param
   * status @param @return 设定文件 @return List<String> 返回类型 @throws
   */
