...
public class RegionController {
  ...
  public boolean deleteRegionbyId(String id, String pid) {
    ...
    if (region.getChildren().size() > 0 || saojieDateService.findByReion(region).size() > 0
        || null != saojieService.findByregion(region) || assessService.findByRegion(region).size() > 0) {
      return false;
    }
  ...
  public Long getMaxId(String pid) {
    ...
    Region region = regionService.findListRegionbyid(pid);
    Long maxid = 0L;
    if (region.getType().getName().equals("国") || region.getType().getName().equals("省")) {
      for (Region reg : region.getChildren()) {
        if (reg.getType().getName().equals("大区") || reg.getType().getName().equals("区")) {
  ...
  /**
   * getProvince:(这里用一句话描述这个方法的作用). <br/>
   * 
   * @author yangqc
   * @return
   * @since JDK 1.8
   */
  @RequestMapping("/getAllRegion/{type}")
  public ResponseEntity<?> getAllRegion(@PathVariable int type) {
    List<Map<String, Object>> regionList = regionService.getAllRegion(type);
    return new ResponseEntity<List<Map<String, Object>>>(regionList, HttpStatus.OK);
  }
  
  @RequestMapping("/getChlid/{parentId}")
