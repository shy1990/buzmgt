...
public class AssessServiceImpl implements AssessService {
  ...
  public Page<Assess> getAssessList(Assess assess, int pageNum, int size, String regionName) {
    ...
    if (assess.getSalesman() != null) {
      if ((null != assess.getSalesman().getJobNum() && !"".equals(assess.getSalesman().getJobNum()))
          || (null != assess.getSalesman().getTruename() && !"".equals(assess.getSalesman().getTruename()))) {
        String serHql = "and s.truename like '%" + assess.getSalesman().getTruename() + "%' or s.job_num='"
            + assess.getSalesman().getJobNum() + "'";
        hql += "" + serHql + " and s.region_id in" + "(SELECT region_id FROM SYS_REGION START WITH name='" + regionName
            + "' CONNECT BY PRIOR region_id=PARENT_ID)";
      } else {
        hql += "and s.region_id in" + "(SELECT region_id FROM SYS_REGION START WITH name='" + regionName
            + "' CONNECT BY PRIOR region_id=PARENT_ID)";
      }
    } else {
      hql += "and s.region_id in" + "(SELECT region_id FROM SYS_REGION START WITH name='" + regionName
    ...
    for (Object obj : q.getResultList()) {
      ...
      if (SalesmanStatus.weihu.equals(ass.getSalesman().getStatus()) && AssessStatus.AGREE.equals(ass.getStatus())) {
        ...
        // Assess a =
        // assessRepository.findByStageAndSalesman("2",ass.getSalesman().getId());//看是否有第二阶段
        String sql = "select count(oi.id) from SYS_REGISTDATA t left join SYS_Assess a on t.user_id=a.user_id left join SJZAIXIAN.SJ_TB_MEMBERS m "
            + "on t.member_id=m.id left join SJZAIXIAN.SJ_TB_ORDER o on m.id=o.member_id left join SJZAIXIAN.SJ_TB_ORDER_ITEMS oi on o.id = oi.order_id "
            + "where t.member_id=m.id and oi.target_type='sku' and o.pay_status='1' and t.user_id='"
            + ass.getSalesman().getId() + "' ";
        sql += "and to_char(o.createtime,'yyyy-mm-dd hh24:mi:ss') BETWEEN '" + ass.getAssessTime() + "' AND '"
            + ass.getAssessEndTime() + "'";
        Query query = em.createNativeQuery(sql);
        BigDecimal str = null;
        List<BigDecimal> resultList = query.getResultList();
        if (resultList != null && resultList.size() > 0) {
        ...
        String sql1 = "select count(*) from (select m.id,count(o.id) total from SYS_REGISTDATA t "
            ...
            + "and to_char(o.createtime,'yyyy-mm-dd hh24:mi:ss') BETWEEN '" + ass.getAssessTime() + "' AND '"
            + ass.getAssessEndTime() + "' " + "Group by m.id) where total >= 2";
        query = em.createNativeQuery(sql1);
        BigDecimal big = null;
        resultList = query.getResultList();
        if (resultList != null && resultList.size() > 0) {
