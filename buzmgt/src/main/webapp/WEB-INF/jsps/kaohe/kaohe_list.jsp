...
<body>
	<div class="main">
		<h4 class="team-member-header page-header ">
			<i class="icon icon-saojie-list"></i>考核开发
			<!--区域选择按钮-->
						...
						<div class="row">
							...
							<div class="col-sm-8 col-md-6">
								...
								<ul class="nav nav-tabs">
									...
									<li title="考核中"><a title="考核中" name="status"
									...
									<li title="考核失败"><a title="考核失败" name="status"
										onclick="getSaojieList(this.title,this.name,${regionId});"
												...
												<c:if test="${not empty list.content}">
													<c:forEach var="saojie" items="${list.content}"
														...
														<tr>
															...
															<td class="project-title"><a href="javascript:toSalesManInfo('${assess.salesman.id}','saojie');"><strong>${assess.salesman.truename}</strong>(${assess.salesman.user.organization.name})</a>
																<br /> <span>${assess.salesman.region.name}</span></td>
															<c:if test="${assess.status == 'PENDING' }">
																<td class="project-status"><span class="status-ing">${assess.status.name}</span></td>
															</c:if>
															<c:if test="${assess.status == 'AGREE' }">
																<td class="project-status"><span
																	class="status-finish">扫街完成</span></td>
															</c:if>
															<td class="project-title"><span class="l-h">${assess.region.name}：<strong
