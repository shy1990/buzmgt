...
<head>
    ...
    <script id="record-table-template" type="text/x-handlebars-template">
        ...
        <tr>
            ...
            <td>{{record}}</td>
            <td>{{offsetNums}}</td>
            <td>{{show record offsetNums}}</td>
            <td>{{amount}}</td>
            <td><button onclick="showDetails('{{userId}}','{{superId}}','{{startTime}}','{{endTime}}')">查看明细</button></td>
        </tr>
        {{/each}}
        {{/if}}
    </script>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        getRecords(time,salesmanId);
    });

    function getRecords(time,salesmanId){
        $.ajax({
            url:'superposition/showRecords',
    ...
    function handelebarsRegister(data){
        ...
        Handlebars.registerHelper("show",function (record,offsetNums) {
            return record - offsetNums;
        });
        $("#record-tbody").html(recordTemplate(data));
    }
//String userId, Long superId, String startTime, String endTime
    function showDetails(userId,superId,startTime,endTime){
        window.location.href = 'superposition/showRecordsDetails?userId=' + userId + '&superId=' + superId + '&startTime=' + startTime + '&endTime=' + endTime;
    }
</script>
