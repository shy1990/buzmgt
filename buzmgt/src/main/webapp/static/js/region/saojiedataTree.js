...
var zNodes = [ {
	name:"扫街数据", font:{'font-weight':'italic'},
	id : "0",
	isParent : true,
	open : true,
...
function ajaxGetNodes(treeNode, reloadType) {
	var zTree = $.fn.zTree.getZTreeObj("saojiedataId");
...
function beforeRemove(treeId, treeNode) {
	...
	if (confirm("确认删除 节点 -- " + treeNode.name + " 吗？")) {
		...
		$.ajax({
			...
			url : "organization/deleteOrganizationbyId", // 请求链接
...
function addHoverDom(treeId, treeNode) {
	...
	if (btn) btn.bind("click", function(){
		...
		$.ajax({
			...
			dataType : "text", // 数据传输格式
			url : "organization/addOrganization", // 请求链接
			error : function() {
...
function onloadZTree() {
	...
	$.ajax({
		...
		type : 'post', // 请求方式,post
		dataType : "text", // 数据传输格式
		url : "/region/findOneSaojieRegion?regionid="+parentid, // 请求链接
		error : function() {
			BootstrapDialog.alert("访问服务器出错");
		},
		success : function(data) {
			...
			$.fn.zTree.init($("#saojiedataId"), setting, zNodes);
			rMenu = $("#rMenu");
		}
	});
}
