 ...
 */
/*
 * function getUrlByNodeId(treeId, treeNode) { return "area/getArea"; }
 */
/**
 * 展开之前执行的函数
 * 
...
function onClick(e, treeId, treeNode) {
	...
	regionObj.append("<option  selected='selected' value = '" + v + "'>" + name
			+ "</option>");
	// hideRegionMenu();
}
...
function onBodyDown(event) {
	...
	if (!(event.target.id == "menuBtn" || event.target.id == "region"
			|| event.target.id == "regionMenuContent" || $(event.target)
			.parents("#regionMenuContent").length > 0)) {
		hideRegionMenu();
	}
}
//
