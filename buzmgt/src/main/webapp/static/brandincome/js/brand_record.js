...
Handlebars.registerHelper('whatUnderwayButton', function(startDate,endDate,status) {
	var html = "";
	var nowDate = getTodayDate();
	if (checkDate(startDate,nowDate) && checkDate(nowDate,endDate) && status === "OVER") {
		html += '<button class="btn btn-sm bnt-jc spc" data-toggle="modal" data-target="#">进程</button>';
		html += '<button class="btn bnt-sm bnt-ck spc" data-toggle="modal" data-target="#">查看</button>';
		html += '<button class="btn  bnt-sm bnt-zza spc" data-toggle="modal" data-target="#">终止</button>';
	}
	if (checkDate(nowDate,startDate) && status === "WAIT") {
		html += '<button class="btn bnt-sm bnt-ck" data-toggle="modal" data-target="#">查看</button>';
	}
	if (checkDate(nowDate,startDate) && status === "BACK") {
		html += '<button class="btn bnt-sm btn-zz spc" data-toggle="modal" data-target="#del">修改</button>';
		html += '<button class="btn bnt-sm btn-sc " data-toggle="modal" data-target="#xgywxx"> 删除</button>';
	}
	if (checkDate(nowDate,startDate) && status === "OVER") {
		html += '<button class="btn btn-sm bnt-jc spc" data-toggle="modal" data-target="#">进程</button>';
		html += '<button class="btn bnt-sm bnt-ck" data-toggle="modal" data-target="#">查看</button>';
	}
	return new Handlebars.SafeString(html);
...
Handlebars.registerHelper('whatExpiredButton', function(startDate,endDate,status) {
	var html = "";
	var nowDate = getTodayDate();
	if (checkDate(endDate,nowDate) && status === "OVER") {
		html += '<button class="btn btn-sm bnt-jc spc" data-toggle="modal" data-target="#">进程</button>';
		html += '<button class="btn bnt-sm bnt-ck spc" data-toggle="modal" data-target="#">查看</button>';
	}
	return new Handlebars.SafeString(html);
