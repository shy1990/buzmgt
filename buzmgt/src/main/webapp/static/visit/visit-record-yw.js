...
/*业务拜访任务列表*/
$(function() {
	searchData['userId'] = $('#box-title').attr("data-a");
	ajaxSearch(searchData);
	
	condition['regionid'] = $("#area").find("option:selected").val();
...
function ajaxSearch(searchData) {
	$.ajax({
		url : base + "visit/visitRecordYWList",
		type : "GET",
		data : searchData,
...
function seeDetails(visitId){
	window.location.href = "/visit/visitDetPage?visitId="+visitId;
}
